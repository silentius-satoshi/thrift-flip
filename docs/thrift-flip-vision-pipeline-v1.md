...
### **This track is now first in the build order** — V0 validation and V1 run before any front-end work (`thrift-flip-plan.md` §6)
...
- **This track goes first.** V0 (an hour of AI Studio validation, no code) then V1 are the next two steps in the whole project, ahead of the design system. §6.2 of the plan defines V0; it scores the same five core items §7's V1 gate names, so the two tests chain.
...
- **V1 now precedes F1**, so anything below that assumes the `ui/` component layer or the new token block is premature. The pencil render in particular is hand-rolled CSS (§2.5).
...
> **This flow needs a Settings screen, and there isn't one.** The repo has no Settings surface at all (plan §3). V1 therefore builds the Settings screen *and* the key-detail sub-screen described below, both joining the lazy-init + persist pattern or a refresh ejects him mid-flow (plan §8). Budget V1 accordingly — plan §6.1 sizes it.
...
2. **Paste field** — waiting on return, `autocomplete="off"`, paste-button beside it (iOS clipboard prompt handles permission). **Set `font-size: 16px` explicitly**: the app's base is still 15px until F1 raises it, and an input at the base zooms the page on focus (plan §8). This is the field that will bite.
...
> **The pencil tag is a real V1 dependency, and it was originally scheduled last.** The break-even arithmetic and the pencil verdict were A-track work (frontend v2 §10), after everything else, while this section and the V1 gate both assume the tag works from first launch. **Resolved in plan §6.1: both halves ship at V1** — the arithmetic (on-device math over the existing `src/utils/calculations.js`, which also gets its first unit tests) *and* a render. **The render is hand-rolled CSS on the existing `VerdictCard`**, because V1 now runs before F1: there is no `ui/` layer to build against and no new token block, so `--t-hero`, the `.money` tabular-figures rule and the `:focus-visible` ring do not exist yet — hardcode their equivalents. F2 rebuilds the verdict properly as `VerdictBanner variant="pencil"` and deletes the interim styling.
...
*(Position in the overall sequence: V0 and V1 are the next two steps in the entire project; T1 — a real trip — sits between V1 and V1.5; V2–V4 run after E1–E2. See `thrift-flip-plan.md` §6.)*

**V0 — Validation, no code (about an hour).** Defined in plan §6.2: 10–15 real thrift items through AI Studio on the §5 schema and the real system prompt, scoring identification, price, and calibration. Gate: **≥4 of the 5 core items** (sneaker, book, tool, mug, vintage electronics) correct on brand+model, defensible price ranges, and confidence that tracks accuracy. Miss it and the fix is in §3 before V1 is written.

**V1 — Real analyze, model-only pricing, the pencil floor, and the first Settings screen.** Gemini structured call wired end-to-end **direct from the client**, comps = tier C only. Plus the pencil arithmetic with its interim `VerdictCard` render, `calculations.js` unit tests, and the §2.5 Settings surface. Gate: **the same 5 core items from V0, re-shot through the app**, ≥4 correct brand+model — V0 tested the model, this tests the wiring against a known baseline; JSON parses with zero cleanup code anywhere in the path; and the §2.5 no-key gate holds end-to-end.

**→ T1 — the first real trip** (plan §6.3). Not a build step in this spec, but it happens here, and what it finds shapes V1.5 and everything in the F-track.

**V1.5 — Marketplace variants + the Vendoo handoff (an hour, rides on V1's code — but scheduled after T1).** Add `listing_mercari` to the responseSchema (one schema block, zero new dependencies — the same call now returns both registers). Client: a "Copy for Mercari" button on the generated listing that copies title + description + hashtags and deep-links to the Mercari app; ListingMode shows which variant is on the clipboard. Document the distribution path in the README: **Send to eBay (our API, one tap) → Vendoo imports the eBay listing → Vendoo's automation crosslists to Mercari/Poshmark/FB.** We never build marketplace automation ourselves — eBay is the API-real spine, Vendoo is an optional fan-out that our listing quality feeds, and the copy-assist covers the no-Vendoo case. Running it after T1 means the listing half gets built against items he actually bought. Gate: analyze one item → both listings present in one response; Copy for Mercari pastes cleanly into the Mercari app's form. *(The third original check — "the eBay draft created by Send-to-eBay appears in Vendoo's import screen untouched" — needs API draft creation, which is E2. It runs as part of E2's gate; at V1.5 the only eBay lane is E0's clipboard.)*
