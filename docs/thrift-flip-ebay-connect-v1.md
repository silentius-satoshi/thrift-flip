...
> **✅ DECIDED (Founder, July 2026): photo-less drafts.** E2 creates drafts without photos; Dad adds them in Seller Hub on the big screen, where he already reviews before publishing. Zero new surface; app photos stay on-device. Revisit only if trips show the Seller Hub photo step is a real friction point (Trading API `UploadSiteHostedPictures` is the fallback). Original question kept below for the trail.
>
