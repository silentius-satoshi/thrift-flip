...
2. **F2+A** as one prompt: the verdict rebuilt as `VerdictBanner + ListingPreviewCard + Panel` (all three variants, pencil included and data-driven), *and* the v3.1 four-tab camera-first shell, both assembled from F1's `ui/` components. Build against the prototype, not from improvisation (§6.1).
